...
<!-- gợi ý: mục 1 là frontmatter; status: draft | review (chờ duyệt) | approved (đã duyệt) | implemented (đã phát hành) | deprecated (bãi bỏ); format: spec | use-case | story | crud | state; has_ui: false thì bỏ mục 6. Một file cho một tính năng. Sơ đồ là mục lục, bảng hành vi là diễn giải từng nút; ràng buộc bằng mã bước B1, B2... Chèn bước giữa B2 và B3 thì đặt B2a, không đánh số lại. Sửa hành vi thì sửa đây trước. Ngoại lệ chiếm ít nhất một nửa giá trị tài liệu. -->
...
<!-- gợi ý: nhiều thành phần tương tác theo thứ tự thì dùng sequenceDiagram; mỗi thông điệp mang mã bước, trùng với bảng luồng chính ở mục 5. -->
...
```
{{else}}
<!-- gợi ý: một sơ đồ cho luồng chính, mỗi nút mang mã bước. Chuỗi hành động có rẽ nhánh: flowchart; nhiều thành phần tương tác theo thứ tự: sequenceDiagram; vòng đời trạng thái: stateDiagram. Luồng tuyến tính dưới bốn bước hoặc CRUD đơn giản thì bỏ sơ đồ, bảng hành vi là đủ. Nhánh rẽ ghi nhãn trên mũi tên; ngoại lệ để mục 7, không nhồi vào đây. -->
...
<!-- gợi ý: một đối tượng, mỗi trạng thái một nút, mỗi chuyển trạng thái ghi sự kiện gây ra; tên trạng thái trùng với cột dữ liệu đổi ở mục 5 -->
...
```
{{end}}{{end}}
{{if eq .Format "crud"}}## 5. Bảng field và quyền

<!-- gợi ý: CRUD đơn giản không cần sơ đồ và use case. Mỗi field một dòng; cột quyền ghi vai trò được xem, sửa. Kết quả mỗi thao tác tạo, sửa, xóa ghi ở mục 7 nếu có ngoại lệ. -->

| Field | Kiểu | Bắt buộc | Quyền xem | Quyền sửa | Ghi chú |
|---|---|---|---|---|---|
| | | | | | |
{{else if eq .Format "use-case"}}## 5. Use Case Specification

<!-- gợi ý: tiền điều kiện và hậu điều kiện kiểm chứng được; bảng luồng chính dùng đúng mã bước trong sơ đồ mục 4, mỗi dòng một cặp hành động và phản hồi quan sát được -->
...
| B3 | | |
| B4 | | |
{{else}}## 5. Hành vi theo mã bước

<!-- gợi ý: mỗi dòng một cặp hành động của tác nhân và phản hồi quan sát được. Không viết "hệ thống xử lý" mà không nói kết quả nhìn thấy: thông báo gì, dữ liệu đổi thế nào, sự kiện nào phát ra. Mã bước trùng 100% với sơ đồ mục 4. -->
...
<!-- gợi ý: mỗi ngoại lệ ghi mã bước phát sinh, điều kiện, phản hồi quan sát được. Mục này chiếm ít nhất nửa số dòng bảng hành vi. Nhánh thay thế cần bước riêng thì đặt mã hậu tố theo bước gốc (B2a, B2b) và thêm vào sơ đồ, bảng mục 5. -->
