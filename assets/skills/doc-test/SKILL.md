...
Soạn Testing strategy, sinh Test case từ mục 8 Feature Spec, checklist test giao
...
| Mục | Cách viết |
|---|---|
| 1 Loại test | unit, integration, e2e, giao diện; mục đích, phạm vi, ai viết; chỉ loại stack đang có hoặc người muốn thêm |
| 2 Công cụ | từ stack thật, ghi phiên bản và lệnh chạy; đề xuất công cụ BDD nếu chưa có |
| 3 Định dạng | `gherkin` khi có (hoặc sẽ có) công cụ BDD, `table` khi không; ghi `format` vào frontmatter |
| 4 Ngưỡng | bao phủ, test chặn merge, test trước phát hành |
| 5 Test giao diện | bỏ mục khi `has_ui: false` toàn dự án |
...
Mở spec người chỉ: cần `status: approved` và mục 8 có `AC1`, `AC2`... Thiếu:
...
`format: table` thì thay `test-case` bằng `test-case-table`. `dk` chép mục 8 thành
một Scenario (hoặc một dòng bảng) cho mỗi AC với tag `@F-xxx @ACn`, mục 2 thành
...
- Thêm Scenario cho từng dòng ngoại lệ mục 6 của spec với tag `@F-xxx @En`; dữ
...
ở mục 4 của test report sau này; không được im lặng.
...
Chỉ khi spec có mục 5 (`has_ui: true`) và mockup ở cột Mockup đã `approved`:

```
dk new ui-test-checklist <slug> --from docs/features/<F-xxx>-<slug>.md --set owner="<người duyệt>"
```

`dk` sinh một dòng `- [ ] Bn: khớp mockup <liên kết>` cho mỗi mã bước ở mục 5.
Khi chạy: so màn hình thật với mockup và Design system theo từng mã bước; lệch
thì ghi mục 2 (mã bước, điểm lệch, ảnh); không tự sửa mockup hay spec. **Dừng**,
...
- Mục 1: phiên bản, môi trường, liên kết file test case đã chạy.
- Mục 2: chạy bộ test, dán output thật của công cụ vào khối ```; bảng đếm từ
  output, không ước lượng.
- Mục 3: mỗi fail một dòng, để trống cột Quyết định và Người.
- Mục 4: ghi "Gherkin chưa kiểm chạy được" nếu bước 4 ở trên chưa làm được.
